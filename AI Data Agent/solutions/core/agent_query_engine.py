...
TOOL_DEFINITIONS = [
    ...
    },
    {
        ...
        "function": {
            "name": "query_scada",
            "description": "查询SCADA/Historian实时监控数据（温度、湿度、压差、压力、流量等）。支持多设备对比、阈值分析、趋势分析。当用户查询涉及洁净区环境参数、设备实时数据、温度/湿度/压差对比时使用此工具。",
            "parameters": {
                "type": "object",
                "properties": {
                    "devices": {
                        "type": "string",
                        "description": "要查询的设备描述，多个用逗号分隔。如'纯化间温度'或'纯化间温度,培养间温度'"
                    },
                    "time_range": {
                        "type": "string",
                    ...
                    "analysis_type": {
                        "type": "string",
                        "description": "分析类型：raw(原始数据)、threshold(阈值分析，需指定threshold参数)、comparison(多设备对比)、trend(趋势分析)",
                        "enum": ["raw", "threshold", "comparison", "trend"]
                    },
                    "threshold": {
                        "type": "number",
                        "description": "阈值，仅在analysis_type=threshold时需要。如22.4表示22.4℃"
                    },
                    "threshold_operator": {
                        "type": "string",
                        "description": "阈值比较运算符：>(超过)或<(低于)，默认>",
                        "enum": [">", "<"]
                    }
                },
                "required": ["devices"]
...
class AgentQueryEngine:
    ...
    def _execute_sql(self, sql: str, connection_name: str) -> Dict:
        try:
            ...
            db_configs = {
                'EAM': {
                    'host': os.getenv('DB_EAM_HOST', '10.103.1.105'),
                    'port': int(os.getenv('DB_EAM_PORT', '1433')),
                    'user': os.getenv('DB_EAM_USER', 'nl2api_readonly'),
                    'password': os.getenv('DB_EAM_PASSWORD', ''),
                    'database': os.getenv('DB_EAM_DATABASE', 'EAM'),
                },
                'WMS_PROD': {
                    'host': os.getenv('DB_WMS_HOST', '10.103.1.105'),
                    'port': int(os.getenv('DB_WMS_PORT', '1433')),
                    'user': os.getenv('DB_WMS_USER', 'nl2api_readonly'),
                    'password': os.getenv('DB_WMS_PASSWORD', ''),
                    'database': os.getenv('DB_WMS_DATABASE', 'WMS_PROD'),
                },
            }
            cfg = db_configs.get(connection_name)
            if not cfg:
                return {"success": False, "error": f"直连不支持连接: {connection_name}"}

            conn = pymssql.connect(
                server=cfg['host'], port=cfg['port'],
                user=cfg['user'], password=cfg['password'],
                database=cfg['database'], charset='utf8', timeout=60)
            ...
            logger.error(f"[AgentQueryEngine] 直连SQL执行失败: {e}")
            return {"success": False, "error": f"直连执行失败: {str(e)[:200]}"}
        ...
        if tool_name == "query_by_template":
            ...
            if result.get("success"):
                ...
                if data:
                    ...
                    if auto_chart:
                        yield auto_chart
                data_summary = json.dumps(data[:50], ensure_ascii=False)
                if len(data) > 50:
            ...
            try:
                ...
                # 解析查询
                full_query = f"{devices_str} {time_range_str}"
            ...
            if tool_calls:
                    ...
                    assistant_msg["tool_calls"] = [
                        {
                            ...
                            "function": {
                                ...
                                "arguments": tc.get("raw_arguments", json.dumps(tc["arguments"], ensure_ascii=False))
                            }
                        }
                ...
                for tc in tool_calls:
                    ...
                    logger.info(
                        ...
                        f"步骤={step.step} | 参数={json.dumps(tool_args, ensure_ascii=False)[:200]}"
                    )
                            ...
                            if event["type"] == "data":
                                ...
                                if analysis_type == "threshold":
                                    summary_text = (
                                        ...
                                        f"原始结果: {json.dumps({k: v for k, v in ar.items() if k != 'periods'}, ensure_ascii=False)}"
                                    )
                                    ...
                                    summary_text += f"原始结果: {json.dumps({k: v for k, v in ar.items() if k not in ('devices', 'chart_series')}, ensure_ascii=False)}"
                                elif analysis_type == "trend":
                                    summary_text = (
                                        f"SCADA趋势分析结果: 趋势{ar.get('trend', '未知')}, "
                                        f"总变化{ar.get('total_change', 0)}, "
                                        f"每小时变化{ar.get('rate_per_hour', 0)}。"
                                        f"原始结果: {json.dumps({k: v for k, v in ar.items() if k != 'hourly_avg'}, ensure_ascii=False)}"
                                    )
                                else:
                    ...
                    if tool_result_data is not None:
                        result_content = json.dumps(tool_result_data, ensure_ascii=False)
                        ...
                        result_content = json.dumps(tool_result_scada, ensure_ascii=False)
                    else:
                        ...
                        if msg.get("role") == "tool" and msg.get("content"):
                            ...
                            try:
                                ...
                                if isinstance(tool_data, list):
                                    ...
                                    if tool_data:
                                        rows = tool_data[:20]
                                        data_facts.append(json.dumps(rows, ensure_ascii=False))
                                        has_valid_data = True
                                    else:
        ...
        for msg in messages:
            if msg.get("role") == "tool" and msg.get("content"):
                ...
                try:
                    ...
                    if isinstance(tool_data, list):
                        if tool_data:
                            data_facts.append(json.dumps(tool_data[:20], ensure_ascii=False))
                    elif isinstance(tool_data, dict):
                        if tool_data.get("type") == "scada_result":
                            data_facts.append(tool_data.get("summary", ""))
